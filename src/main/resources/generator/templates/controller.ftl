...
public class ${className}Controller extends BaseController {
    ...
    @GetMapping(FebsConstant.VIEW_PREFIX + "gen/${className?uncap_first}")
    @RequiresPermissions("${className?uncap_first}:list")
    private String ${className?uncap_first}Index(){
        return FebsUtil.view("gen/${className?uncap_first}/${className?uncap_first}");
    }
	@GetMapping(FebsConstant.VIEW_PREFIX + "gen/${className?uncap_first}/add")
   	@RequiresPermissions("${className?uncap_first}:add")
   	public String ${className?uncap_first}Add() {
   		return FebsUtil.view("gen/${className?uncap_first}/${className?uncap_first}Add");
   	}

   	@GetMapping(FebsConstant.VIEW_PREFIX + "gen/${className?uncap_first}/update/{${className?uncap_first}Id}")
   	@RequiresPermissions("${className?uncap_first}:update")
   	public String ${className?uncap_first}Update(@PathVariable Integer ${className?uncap_first}Id, Model model) {
   		...
   		return FebsUtil.view("gen/${className?uncap_first}/${className?uncap_first}Update");
   	}
    ...
    @Log("删除${className}")
    @GetMapping("${className?uncap_first}/delete/{${className?uncap_first}Ids}")
    @ResponseBody
    @RequiresPermissions("${className?uncap_first}:delete")
    public FebsResponse delete${className}(@NotBlank(message = "{required}") @PathVariable String ${className?uncap_first}Ids) throws FebsException {
        try {
            String[] ids = ${className?uncap_first}Ids.split(StringPool.COMMA);
            List<String> list = Arrays.asList(ids);
            this.${className?uncap_first}Service.removeByIds(list);
            return new FebsResponse().success();
        } catch (Exception e) {
